...
def _parse_diag_photo_nums(text):
    ...
    対応フォーマット:
      通常:       「写真番号1-5」
      前回括弧付: 「写真番号1-32(1-20)」→ 括弧内は除外
      カンマ連番: 「写真番号1-34,35」「写真番号1-34，35」
      範囲表記:   「写真番号1-28～1-36」「写真番号1-28~1-36」
      OCR誤読:    「写真番号1-321-20」（括弧消え）→ 後半除去

    OCRテキストに「補修箇所」または「補修済み」が含まれる場合は
    補修箇所ページと判断して None を返す（リンク対象外）。
    """
    text = _normalize_text(text)
    # 全角カンマ・全角チルダを半角に統一
    text = text.replace('，', ',').replace('～', '~')
    ...
    # ── ①補修箇所ページ検出 ──
    if '補修箇所' in text or '補修済み' in text or '補修済' in text:
        return None

    # ── ②前処理: 前回点検の括弧内写真番号を除去 ──
    # 正常OCR: 「写真番号1-32(1-20)」→ 括弧ごと除去
    text = re.sub(r'\(\d+-\d+\)', '', text)
    # OCR誤読: 「写真番号1-321-20」のように括弧が消えた場合、後ろの「\d+-\d+」を除去
    text = re.sub(r'(写真番号\s*\d+\s*[-－]\s*\d+)\s*\d+\s*[-－]\s*\d+', r'\g<1>', text)

    results = set()

    # ── ③範囲表記「写真番号N-A～N-B」を展開 ──
    for m in re.finditer(
        r'写真番号\s*(\d+)\s*[-－]\s*(\d+)\s*[~～]\s*\s*[-－]\s*(\d+)',
        text
    ):
        span  = int(m.group(1))
        start = int(m.group(2))
    ...
    # ── ④通常表記＋カンマ連番「写真番号N-M,X,Y」を取得 ──
    # 範囲表記はすでに処理済みなので重複しても set で吸収される
    for m in re.finditer(
        r'写真番号\s*(\d+)\s*[-－]\s*(\d+)((?:\s*,\s*\d+)*)',
        text
    ):
        span = int(m.group(1))
        results.add((span, int(m.group(2))))
        for extra in re.findall(r'\d+', m.group(3)):
            results.add((span, int(extra)))
    ...
    try:
        ...
        for didx, plist in diag_to_photo_pages.items():
            ...
            for pp in plist:
                ...
                span_prefix = f"{span}-" if (is_multi_span and span) else ""
                label = (f"→ {span_prefix}{min(photo_nums)}~{max(photo_nums)}"
                         if photo_nums else f"→ p.{pp+1}")
